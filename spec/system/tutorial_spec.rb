...
RSpec.describe "튜토리얼 (W7-T04)", type: :request do
  ...
  describe "수동 진행 (POST /tutorial/next)" do
    it "step 1 → 2로 advance" do
      post "/tutorial/next"
      expect(Sowing::Core::Settings.load["tutorial_step"]).to eq(2)
    end

    it "step 4에서 next → tutorial_completed_at 마킹" do
      Sowing::Core::Settings.update(tutorial_step: 4)
      post "/tutorial/next"
      expect(Sowing::Core::Settings.load["tutorial_completed_at"]).to match(/\A\d{4}-\d{2}-\d{2}T/)
    end
  end
  ...
  describe "POST /tutorial/skip" do
    it "tutorial_completed_at 즉시 마킹 + 대시보드로 redirect" do
      post "/tutorial/skip"
      expect(Sowing::Core::Settings.load["tutorial_completed_at"]).not_to be_nil
  ...
  describe "POST /tutorial/restart" do
    it "step 1로 리셋 + completed_at clear" do
      Sowing::Core::Settings.update(tutorial_step: 4, tutorial_completed_at: "2026-01-01T00:00:00+09:00")
